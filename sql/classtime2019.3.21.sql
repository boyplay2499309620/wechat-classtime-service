...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 COMMENT='保留用户设备信息';
...
) ENGINE=InnoDB AUTO_INCREMENT=16 DEFAULT CHARSET=utf8 COMMENT='存放所有人的时间总记录';
...
) ENGINE=InnoDB AUTO_INCREMENT=123 DEFAULT CHARSET=utf8 COMMENT='用于保存房间本身的基本信息，保留一条系统记录，用作表示未加入房间的情况，id为1，state永远为1';
...
) ENGINE=InnoDB AUTO_INCREMENT=86 DEFAULT CHARSET=utf8 COMMENT='保存加入房间的人的信息，每个加入房间的人都会创建一条记录，以后可更新times字段';
...
CREATE TABLE `test` (
  ...
  PRIMARY KEY (`ids`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='用来做测试的表，可任意更改';
...
) ENGINE=InnoDB AUTO_INCREMENT=142 DEFAULT CHARSET=utf8 COMMENT='保存每按下stop按钮所提交的记录，即便同一房间类也可能产生多条记录';
